...
class IngestionPipelines:
    ...
    def run_etl_02_type_casting_nightmare(self):
        ...
        raw_df = (
            ...
            .csv(f"{self.source_path}/pv_metrics_raw")
        )
    ...
    def run_etl_03_missed_broadcast(self):
        ...
        df_facts.write.mode("overwrite").format("delta").saveAsTable(
            f"{self.target_schema}.bess_facts_raw"
        )
        ...
        mock_data = [
            ("S001", "Warsaw_Center"),
            ("S002", "Brwinow_West"),
            ("S003", "Grodzisk_Mazowiecki"),
        ]
        df_dim = self.spark.createDataFrame(mock_data, ["station_id", "station_name"])
        df_dim.write.mode("overwrite").format("delta").saveAsTable(
            f"{self.target_schema}.stations_dict"
        )
        ...
        """)
        logger.info(
            f"✅ ETL 3 completed. Created dynamic view: {self.target_schema}.enriched_telemetry_heavy_shuffle"
        )
    ...
    def run_etl_04_over_partitioning_and_skew(self):
        ...
        bad_df.write.mode("overwrite").format("delta").partitionBy("bad_partition_key").saveAsTable(
            target_table
        )
