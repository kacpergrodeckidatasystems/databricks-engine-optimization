...
class EventLogReader(IMetricsReader):
    ...
    def get_execution_plan(self) -> str:
        ...
        plan_row = logs_df.filter(F.col("Event") == "SparkListenerSQLExecutionStart") \
                          .select("physicalPlanDescription") \
                          .orderBy(F.col("time").desc()) \
                          .first()
        return plan_row["physicalPlanDescription"] if plan_row and plan_row["physicalPlanDescription"] else ""
    ...
    def get_physical_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {"max_task_duration_ms": 0, "min_task_duration_ms": 0, "num_files": 0, "total_size_bytes": 0}
