...
class TypeCastingRule(IAnalysisRule):
    def evaluate(
        ...
        if not plan_text:
            ...
            if detected_anomalies:
                return Alert(
                    ...
                    severity="WARNING",
                    metrics_captured={
                        "anomalies": list(set(detected_anomalies)),
                        "total_count": len(set(detected_anomalies)),
                    },
                )
            return None
        ...
        for line in lines:
            ...
            if any(
                k in line_lower
                for k in ["filter", "datafilters", "partitionfilters", "photonfilter"]
            ):
                matches = cast_pattern.findall(line)
                for col_name, target_type in matches:
                    detected_anomalies.append(
                        f"active filter casting on column '{col_name}' to type '{target_type}'"
                    )
        ...
        for line in lines:
            ...
            if any(k in line_lower for k in ["relation", "read", "scan", "localrelation"]):
                ...
                for col_name in matches:
                    ...
                    if any(keyword in col_name.lower() for keyword in METRIC_KEYWORDS):
                        detected_anomalies.append(
                            f"IoT metric '{col_name}' permanently stored as STRING in table metadata"
                        )
